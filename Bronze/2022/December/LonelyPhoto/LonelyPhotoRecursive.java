...
public class LonelyPhotoRecursive {
    public static long countLonely(String characters) {
        ...
        // merge the results by counting the number of "lonely" sub-strings that span across the two halves
        long crossLonely = 0;
        for (int i = 0; i < n / 2; i++) {
            if (left.charAt(i) == right.charAt(0) && (right.length() == 1 || right.charAt(1) != right.charAt(0))) {
                // count sub-strings of the form "HG" or "GG"
                crossLonely++;
            }
            if (right.charAt(0) == left.charAt(i) && (left.length() == 1 || left.charAt(i + 1) != left.charAt(i))) {
